...
public void setup() {
    ...
    for (int i = 0; i < technologies.length - 1; i++) {  // Boucle pour placer les technologies aléatoirement sauf la dernière
        if (i != 5) { // Sans la dernière technologie
            int randomX = PApplet.parseInt(random(50, width - 50));  // X aléatoire dans le jardin
            int randomY = PApplet.parseInt(random(jardinY, jardinY + jardinLength));  // Y aléatoire dans le jardin
...
class Technologie {
    ...
    public void display(float zoom) { //l'affichage de la technologie est blurry, je vais regarder comment régler ça
        // Calcul de la nouvelle largeur et hauteur de l'image
        int newWidth = PApplet.parseInt(techImages[imageIndex].width * zoom);
        int newHeight = PApplet.parseInt(techImages[imageIndex].height * zoom);
